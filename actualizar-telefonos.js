...
async function runUpdate() {
  ...
  try {
      ...
      for (const lead of leads) {
        ...
        if (phoneField && phoneField.values.length > 0) {
          ...
          if (mainPhoneNumber && mainPhoneNumber !== searchPhoneNumber) {
            console.log(
              `(${totalProcessed}) Actualizando Lead ID ${lead.item_id}: ${mainPhoneNumber}`,
            );
    ...
    if (err.response && err.response.data && err.response.data.error === 'rate_limit') {
      ...
      console.error(
        "Por favor, espera una hora y vuelve a ejecutar 'node actualizar-telefonos.js'. Continuará desde donde se quedó.",
      );
    } else {
